...
class CurrentRecipeFragment : Fragment() {
    ...
    override fun onCreateView(
    ...
    ) = CurrentRecipeFragmentBinding.inflate(
        ...
        with(binding) {
            ...
            viewModel.data.observe(viewLifecycleOwner) { listRecipe ->
                ...
                render(currentRecipe)
                with(adapter) {
                    submitList(currentRecipe.content)
                    differ.submitList(currentRecipe.content)
                }
            }

            viewModel.recipeEditOrAddScreenEvent.observe(
                viewLifecycleOwner
            ) { initialContent ->
                val direction =
            ...
            viewModel.stepEditScreenEvent.observe(
                viewLifecycleOwner
            ) { currentRecipe ->
                val direction =
            ...
            viewModel.stepAddScreenEvent.observe(
                viewLifecycleOwner
            ) { stepText ->
                val direction =
                    CurrentRecipeFragmentDirections.currentRecipeFragmentToStepContentFragment(
